...
type Project struct {
	...
	ReportType  string    `json:"report_type,omitempty"` // enterprise | edusrc
...
func (db *DB) CreateProject(p *Project) (*Project, error) {
	...
	if strings.TrimSpace(p.ReportType) == "" {
		p.ReportType = "enterprise"
	}
	now := time.Now()
...
func (db *DB) GetProject(id string) (*Project, error) {
	...
	err := db.QueryRow(
		`SELECT id, name, COALESCE(description,''), COALESCE(scope_json,''), COALESCE(report_type,'enterprise'), status, pinned, created_at, updated_at
		 FROM projects WHERE id = ?`, id,
	).Scan(&p.ID, &p.Name, &p.Description, &p.ScopeJSON, &p.ReportType, &p.Status, &pinned, &createdAt, &updatedAt)
	if err != nil {
...
func (db *DB) ListProjects(status, search string, limit, offset int) ([]*Project, error) {
	...
	for rows.Next() {
		...
		var pinned int
		var createdAt, updatedAt string
		if err := rows.Scan(&p.ID, &p.Name, &p.Description, &p.ScopeJSON, &p.ReportType, &p.Status, &pinned, &createdAt, &updatedAt); err != nil {
			return nil, err
		}
		p.Pinned = pinned != 0
		p.CreatedAt = parseDBTime(createdAt)
...
func (db *DB) UpdateProject(p *Project) error {
	...
	_, err := db.Exec(
		`UPDATE projects SET name = ?, description = ?, scope_json = ?, status = ?, report_type = ?, pinned = ?, updated_at = ? WHERE id = ?`,
		p.Name, p.Description, p.ScopeJSON, p.Status, p.ReportType, boolToInt(p.Pinned), p.UpdatedAt, p.ID,
	)
	if err != nil {
